...
func NewTextView() *TextView {
	return &TextView{
		...
		align:         AlignLeft,
		wrap:          true,
		baseStyle:     tcell.StyleDefault.Foreground(Styles.PrimaryTextColor),
		regions:       false,
		dynamicColors: false,
	}
}
...
func (t *TextView) SetTextColor(color tcell.Color) *TextView {
	t.baseStyle = t.baseStyle.Foreground(color)
	return t
}

func (t *TextView) SetBackgroundColor(color tcell.Color) *TextView {
	t.baseStyle = t.baseStyle.Background(color)
	return t
}
	...
	for line := t.lineOffset; line < len(t.index); line++ {
		...
		iterateString(strippedText, func(main rune, comb []rune, textPos, textWidth, screenPos, screenWidth int) bool {
			...
			for offset := screenWidth - 1; offset >= 0; offset-- {
				if offset == 0 {
					screen.SetContent(posX+offset, line-t.lineOffset, main, comb, style)
				} else {
					screen.SetContent(posX+offset, line-t.lineOffset, ' ', nil, style)
				}
			}
